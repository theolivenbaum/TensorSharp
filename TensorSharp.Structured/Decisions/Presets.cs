...
//
// The question sets are Laya's (https://github.com/NandhaKishorM/laya, Apache-2.0), as ported in the Laya .NET
// library, so the same workflow can be asked of either engine and the answers compared.
using System;
...
{
    /// <summary>Ready-to-use question sets for common decision workflows - the same sets Laya ships.</summary>
    public static class Presets
    {
        ...
        {
            var options = categories?.ToArray() ?? new[]
            {
        ...
        public static IReadOnlyList<string> Names { get; } = new[] { "triage", "email", "guard", "moderation", "router" };
    }
}
